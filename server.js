...
app.post('/response', async function (req, res) {
  ...
  if (calchash == pd.hash && pd.status== 'success' && pd.unmappedstatus=='captured' && pd.error=='E000') {
    ...
    if (recharging.status == 1) {
      ...
      return res.redirect("https://wingotrading.in/api/response-recharge/" + token);
    }
...
app.post('/response', async function (req, res) {
  ...
  if (calchash == pd.hash) {
    ...
    res.send({ 'status':pd.status,'pd': pd, redirectStatus:recharging.status,redirectURL:"https://www.googleclubs.in/api/response-recharge/" + token });
  } else {
    res.send({ 'status': "Error occured" });
  }
